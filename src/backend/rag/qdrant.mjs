...
export async function getQdrantClient() {
    try {
        ...
        if (process.env.QDRANT_URL && process.env.QDRANT_API_KEY) {
            ...
            console.log('Configurazione collezione:', JSON.stringify(collectionInfo, null, 2));
        ...
        console.error('❌ Risposta server:', error.response?.data || 'Nessuna risposta');
        return null;
    }
}
...
export async function insertDocuments(documents, organizationId) {
    try {
        ...
        const points = documents.map((doc, index) => {
            ...
            const numericId = typeof doc.id === 'string' ? 
                parseInt(Date.now().toString() + index.toString()) : doc.id;
        ...
        // Log per debugging
        console.log(`Tentativo di inserimento di ${points.length} documenti per organizationId: ${organizationId}`);
        ...
        for (let i = 0; i < points.length; i += BATCH_SIZE) {
            ...
            console.log(`✅ Batch ${Math.floor(i/BATCH_SIZE) + 1}/${Math.ceil(points.length/BATCH_SIZE)} inserito`);
        }
        
        console.log(`✅ Inseriti ${points.length} documenti per organizationId: ${organizationId}`);
        return true;
    } catch (error) {
        console.error('❌ Errore durante l\'inserimento dei documenti:', error.message);
        if (error.response) {
...
export async function getDocumentsForUser(organizationId) {
    try {
        ...
        console.error('❌ Errore durante il recupero dei documenti:', error.message);
        if (error.response) {
            console.error('Dettagli errore:', error.response.data);
        }
        return [];
