...
import { Container, Row, Col, Card, Button, Form, ListGroup, Navbar, Nav, Modal, Table, Badge } from "react-bootstrap";
...
const Manager = ({ onLogout }) => {
  ...
  const [customDefaultResponse, setCustomDefaultResponse] = useState("Grazie per il tuo messaggio! Un operatore ti risponderà a breve."); // Risposta automatica
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const updatedDocsResponse = await axios.get(`/api/documents/${viewingDocumentsFor}`);
            ...
            {loading ? (
              ...
              <ListGroup className="conversation-list">
                {Array.isArray(conversations) ? conversations.map((conv) => (
                  <ListGroup.Item key={conv.phone} className="p-0 border" action onClick={() => selectConversation(conv.phone)}>
          ...
          <Modal.Title>
            ...
            {viewingDocumentsFor && (
              <span className="ms-2 text-muted">
                {conversations.find(c => c.phone === viewingDocumentsFor)?.name || viewingDocumentsFor}
              </span>
            )}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          {viewingDocumentsFor && userDocuments[viewingDocumentsFor] ? (
            userDocuments[viewingDocumentsFor].length > 0 ? (
              ...
              <p className="text-center">Nessun documento caricato per questo utente.</p>
            )
          ) : (
